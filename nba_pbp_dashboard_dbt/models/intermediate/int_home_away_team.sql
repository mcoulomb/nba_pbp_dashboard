...
select home_away_array[offset(0)] as temp_struct,game_id from (
SELECT array_agg(struct(home_team_temp, away_team_temp, action_number)) as home_away_array, game_id from (
select    
                CASE
                    WHEN score_diff = home_score THEN team_tricode
                    ELSE null
                END as home_team_temp,  
                CASE
                    WHEN score_diff = away_score THEN team_tricode
                    ELSE null
                END as away_team_temp,
            game_id,
            action_number
    from int_data
     where shot_result = 'made' order by game_id, action_number
) GROUP BY game_id
)
),
