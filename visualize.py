...
# Create hexbin mapbox using figure_factory
    ...
    df_cleaned["y_rounded"] = df_cleaned["y"].round(0)
    distinct_values = df_cleaned["y_rounded"].unique()
...
# Create hexbin mapbox using figure_factory
    ...
    hexbin_t = ax.hexbin(shot_chart_df_t["y_location_converted"], shot_chart_df_t["x_location_converted"],C=shot_chart_df_t["is_shot_attempt"],reduce_C_function=reduce_hexbins_shot_distribution, gridsize=20, cmap="Reds", mincnt=shot_count_filter)
    ...
    hexbin_diff = ax.hexbin(df_cleaned["y"], df_cleaned["x"],C=df_cleaned['shooting_percentage_from_average'], cmap="coolwarm",gridsize=gridsize, vmin=-.05,vmax=.05)
...
def add_court(ax):
    ...
    # Center at hoop (0,0), radius 23'9" (475 units), theta1=22, theta2=158
